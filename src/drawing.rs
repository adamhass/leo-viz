...
pub struct Draw3dInput<'a> {
    pub id: &'a str,
    pub constellations: &'a [DrawConstellationData],
    pub flags: View3DFlags,
    pub coverage_angle: f64,
    pub rotation: Matrix3<f64>,
    pub satellite_rotation: Matrix3<f64>,
    pub width: f32,
    pub height: f32,
    pub earth_texture: Option<&'a egui::TextureHandle>,
    pub zoom: f64,
    pub sat_radius: f32,
    pub link_width: f32,
    ...
    pub body_key: (CelestialBody, Skin, TextureResolution),
    pub body_rotation: &'a Matrix3<f64>,
    pub sun_dir: [f32; 3],
    pub time: f64,
    pub ground_stations_locked: bool,
    pub device_layers: &'a [DeviceLayer],
    pub body_rot_angle: f64,
    pub drag_tab_planet: (usize, usize),
    pub detail_bounds: Option<[f32; 4]>,
    pub geo_borders: &'a [Vec<(f64, f64)>],
    pub geo_cities: &'a [CityLabel],
    pub conjunction_lines: &'a [(ConjunctionInfo, f64)],
    pub conjunction_heatmap: &'a [(ConjunctionInfo, f64)],
    pub correcting_sats: &'a HashSet<String>,
    pub hit_sats: &'a HashSet<String>,
    pub radiation: Option<&'a RadiationConfig>,
    pub moon_handles: &'a HashMap<CelestialBody, egui::TextureHandle>,
    pub physics_colors: &'a HashMap<(usize, usize), egui::Color32>,
    pub physics_info: &'a HashMap<(usize, usize), (f64, f64, bool)>,
    pub ground_tracks: &'a [Vec<(f64, f64)>],
    pub flash_intensities: &'a [HashMap<u32, f32>],
}

pub struct Draw3dState<'a> {
    pub pending_cameras: &'a mut Vec<SatelliteCamera>,
    pub camera_id_counter: &'a mut usize,
    pub satellite_cameras: &'a mut [SatelliteCamera],
    pub cameras_to_remove: &'a mut Vec<usize>,
    pub pinned_isls: &'a mut HashSet<crate::config::PinnedIsl>,
    pub ground_stations: &'a mut [GroundStation],
    pub areas_of_interest: &'a mut [AreaOfInterest],
    pub dragging_place: &'a mut Option<(usize, usize, bool, usize)>,
    pub context_menu_request: &'a mut Option<(egui::Pos2, f64, f64)>,
    pub label_click_request: &'a mut Option<(bool, usize, egui::Pos2)>,
}
